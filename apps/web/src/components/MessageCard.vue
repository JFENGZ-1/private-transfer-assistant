...
import { ArchiveRestore, Combine, Copy, Download, Ellipsis, Eye, FileArchive, FileText, Heart, Image, ListChecks, Lock, Pencil, Pin, QrCode, RotateCcw, Share2, Star, Tag, Trash2, Unlock } from 'lucide-vue-next'
...
const props = withDefaults(defineProps<{ message: Message; trash?: boolean; selectable?: boolean; selected?: boolean; showTime?: boolean; chat?: boolean; selectFromMenu?: boolean }>(), { showTime: true })
...
const menu = ref(false); const menuWrap = ref<HTMLElement>(); const menuX = ref(0); const menuY = ref(0); const shareOpen = ref(false); const expiresIn = ref(3600); const maxDownloads = ref<number | null>(1); const code = ref(''); const sharing = ref(false); const shareUrl = ref(''); let longPressTimer=0; let pressX=0; let pressY=0
...
const menuStyle = computed(() => props.chat ? { left: `${Math.max(8,Math.min(menuX.value,window.innerWidth-286))}px`, top: `${Math.max(8,Math.min(menuY.value-18,window.innerHeight-520))}px` } : undefined)
...
function openMenuAt(x:number,y:number){menuX.value=x;menuY.value=y;menu.value=true;navigator.vibrate?.(15)}
function beginLongPress(event:PointerEvent){if(!props.chat||props.selectable||event.button!==0||(event.target as HTMLElement).closest('button,a,input,textarea,select'))return;pressX=event.clientX;pressY=event.clientY;window.clearTimeout(longPressTimer);longPressTimer=window.setTimeout(()=>openMenuAt(event.clientX,event.clientY),480)}
function moveLongPress(event:PointerEvent){if(Math.hypot(event.clientX-pressX,event.clientY-pressY)>9)window.clearTimeout(longPressTimer)}
function cancelLongPress(){window.clearTimeout(longPressTimer)}
function openContextMenu(event:MouseEvent){if(!props.chat)return;event.preventDefault();openMenuAt(event.clientX,event.clientY)}
...
onUnmounted(() => { window.clearTimeout(longPressTimer); document.removeEventListener('pointerdown', closeMenuOnOutside); imageObserver?.disconnect() })
</script>

<template>
  <article class="message-card" :class="{ private: message.visibility === 'trusted_only', selected, chat, own: chat }" @click="selectable && $emit('select', message.id)" @pointerdown="beginLongPress" @pointermove="moveLongPress" @pointerup="cancelLongPress" @pointercancel="cancelLongPress" @contextmenu="openContextMenu">
    ...
    <div ref="menuWrap" class="message-menu-wrap" @click.stop>
      <button v-if="!chat" class="icon-button message-more" aria-label="消息操作" @click="menu = !menu"><Ellipsis :size="20" /></button>
      <Transition name="fade"><div v-if="menu" class="context-menu" :class="{ 'longpress-menu': chat }" :style="menuStyle">
...
.context-menu.longpress-menu button {
  min-height: auto;
  flex-direction: row;
...
.context-menu.longpress-menu button:hover { background: var(--surface-2); }
.context-menu.longpress-menu button.danger { color: var(--danger); }
