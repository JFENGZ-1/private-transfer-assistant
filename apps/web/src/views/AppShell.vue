...
import { Heart, Inbox, LogOut, Pin, Search, Settings, Share2, Trash2, UserRound, Waves, X } from 'lucide-vue-next'
...
const pinnedCount = computed(() => state.messages.filter(message => message.pinned).length)
...
watch(() => route.path, path => { drawerOpen.value = false; if (path !== '/app') ui.inboxPinnedOpen = false })
...
<template><div class="shell">
  ...
  <main class="shell-main"><header class="mobile-header"><button class="mobile-brand" aria-label="打开导航菜单" :aria-expanded="drawerOpen" @click="drawerOpen = true"><Waves :size="21" /><strong>渡口</strong></button><div class="mobile-header-title"><h1 v-if="route.path === '/app'" class="mobile-page-title">传输助手</h1></div><div class="mobile-header-tools"><button v-if="route.path === '/app' && pinnedCount" class="icon-button mobile-pinned-toggle" :class="{ active: ui.inboxPinnedOpen }" :aria-label="`置顶消息，共 ${pinnedCount} 条`" :aria-expanded="ui.inboxPinnedOpen" @click="ui.inboxPinnedOpen = !ui.inboxPinnedOpen"><Pin :size="19" /><span>{{ pinnedCount }}</span></button><button class="icon-button" aria-label="搜索" @click="openSearch"><Search :size="21" /></button></div></header><RouterView /></main>
