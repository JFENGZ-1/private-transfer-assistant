...
async function submit() {
  ...
  try {
    if (setup.value) {
      if (mainPassword.value.length < 8 || adminPassword.value.length < 8)
        throw new Error("两个口令都至少需要 8 位");
      if (mainPassword.value === adminPassword.value)
        throw new Error("主口令和管理口令不能相同");
    ...
    await router.replace(
      typeof route.query.next === "string" ? route.query.next : "/app",
    );
  } catch (reason) {
...
<template>
  <main class="auth-page">
    <section class="auth-card">
      ...
      <h1>{{ setup ? `初次设置${siteAppearance.siteTitle}` : `回到你的${siteAppearance.siteTitle}` }}</h1>
      <p class="auth-lead">
        {{
          setup
            ? "设置主口令与独立的管理口令。"
      ...
      <form class="auth-form" @submit.prevent="submit">
        ...
        <button class="primary-button full" :disabled="loading">
          {{ loading ? "请稍候…" : setup ? "完成设置并进入" : "临时进入" }}
        </button>
      </form>

      <div class="auth-note">
        <ShieldCheck :size="17" /><span
          >本次为临时会话，刷新或关闭标签页后自动退出。进入后可使用管理口令设为长期设备。</span
        >
      </div>
    </section>
  </main>
