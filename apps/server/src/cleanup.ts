...
export async function runCleanup(app:FastifyInstance){
  ...
  if(candidates.length){const ids=candidates.map(x=>x.id),placeholders=ids.map(()=>'?').join(',');app.db.transaction(()=>{app.db.prepare(`UPDATE messages SET deleted_at=?,updated_at=? WHERE id IN (${placeholders})`).run(now,now,...ids);app.db.prepare(`UPDATE shares SET revoked_at=? WHERE (message_id IN (${placeholders}) OR id IN (SELECT share_id FROM share_messages WHERE message_id IN (${placeholders}))) AND revoked_at IS NULL`).run(now,...ids,...ids);})();for(const row of candidates)app.broadcast({type:'message.deleted',id:row.id});}
  const purged=trashDays>0?await purgeDeleted(app,now-trashDays*day,200):{deletedMessages:0,deletedBlobs:0};
  app.db.prepare('DELETE FROM download_tokens WHERE expires_at<=?').run(now);app.db.prepare('DELETE FROM preview_tokens WHERE expires_at<=?').run(now);app.db.prepare('DELETE FROM sessions WHERE expires_at<=? OR revoked_at IS NOT NULL').run(now);app.db.prepare('DELETE FROM shares WHERE (expires_at>0 AND expires_at<=?) OR revoked_at IS NOT NULL').run(now);app.db.prepare('DELETE FROM drops WHERE expires_at<=? OR revoked_at IS NOT NULL').run(now);
